...
export function drawDetections(canvas, detections, matchByDetection = new Map()) {
  const context = canvas.getContext("2d");
  context.clearRect(0, 0, canvas.width, canvas.height);

  for (const detection of detections) {
    const match = matchByDetection.get(detection);

    drawBoundingBox(context, detection, match);
    drawKeypoints(context, detection, canvas.width, canvas.height);
  }
}

function drawBoundingBox(context, detection, match) {
  ...
  if (typeof score === "number" || match) {
    const faceLabel = typeof score === "number" ? `Face ${(score * 100).toFixed(1)}%` : "Face";
    const matchLabel = match ? ` • ${match.label} ${(match.confidence * 100).toFixed(0)}%` : "";
    const label = `${faceLabel}${matchLabel}`;
