    ...
    <div style={{ display: 'block', maxWidth: '800px', margin: '40px auto', padding: '0 20px' }}>
      <header style={{ marginBottom: '30px' }}>
        ...
        <p style={{ color: '#94a3b8', margin: 0, fontSize: '14px' }}>
          TensorFlow.js WebGL-accelerated convolutional pipeline executing linear vector scan matching against CelebA matrices.
        </p>
      </header>

      <div style={{ background: '#1e293b', padding: '15px', borderRadius: '8px', marginBottom: '20px', fontSize: '14px', borderLeft: '4px solid #3b82f6' }}>
        <strong>Status Node:</strong> {engineState.status}
      </div>

      <div style={{ position: 'relative', width: '100%', maxWidth: '640px', height: '480px', background: '#020617', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.5)' }}>
        <video
          ref={videoRef}
          autoPlay
          muted
        ...
        <canvas
          ref={canvasRef}
          style={{ position: 'absolute', width: '100%', height: '100%', objectFit: 'cover', transform: 'scaleX(-1)', zIndex: 2 }}
        />
      </div>

      <div style={{ marginTop: '25px', background: '#1e293b', padding: '20px', borderRadius: '12px' }}>
        <h3 style={{ margin: '0 0 10px 0', fontSize: '16px', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Classification Output
        </h3>
        <div style={{ fontSize: '22px', fontWeight: '600' }}>
          Identity: <span style={{ color: matchResult.distance && matchResult.distance < 0.6 ? '#4ade80' : '#f87171' }}>{matchResult.label}</span>
        </div>
        {matchResult.distance !== null && (
          <div style={{ marginTop: '5px', color: '#94a3b8', fontSize: '14px' }}>
            Euclidean Vector Metric Space Distance: <code style={{ color: '#e2e8f0', background: '#334155', padding: '2px 6px', borderRadius: '4px' }}>{matchResult.distance.toFixed(5)}</code>
          </div>
        )}
      </div>
    </div>
